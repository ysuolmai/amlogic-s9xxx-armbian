    ...
    def display_info(self, font, font_zh):
        ...
        try:
            with canvas(self.device) as draw:
                ...
                # 第1行: IP地址
                interface_name, ip_address = get_ip_address(self.config['network_interface'])
                net_info = f"{interface_name}:{ip_address}"
...
def _get_interface_ipv4(interface, all_addrs=None):
    """获取指定网络接口的第一个IPv4地址"""
    all_addrs = all_addrs or psutil.net_if_addrs()
    addrs = all_addrs.get(interface, [])
    for addr in addrs:
...
def _get_default_route_interface():
    ...
    try:
        ...
        logger.warning(f"获取默认路由接口失败: {e}")
    return None

def _is_ignored_interface(interface):
    ignored_prefixes = ('lo', 'docker', 'br-', 'veth', 'lxc')
    return any(interface.startswith(prefix) for prefix in ignored_prefixes)

def get_ip_address(interface):
    """获取网络接口和IP地址，支持auto自动选择"""
    try:
        all_addrs = psutil.net_if_addrs()
        preferred = (interface or 'auto').strip()

        if preferred.lower() not in ('auto', 'default'):
            ip_address = _get_interface_ipv4(preferred, all_addrs)
            if ip_address:
                return preferred, ip_address
            logger.warning(f"网络接口 '{preferred}' 没有IPv4地址，尝试自动选择")

        default_interface = _get_default_route_interface()
        if default_interface:
            ip_address = _get_interface_ipv4(default_interface, all_addrs)
            if ip_address:
                return default_interface, ip_address

        for name in all_addrs:
            if _is_ignored_interface(name):
                continue
            ip_address = _get_interface_ipv4(name, all_addrs)
            if ip_address:
                return name, ip_address

        return preferred, "ip:N/A"
    except Exception as e:
        logger.error(f"获取IP地址失败: {e}")
        return interface or "auto", "ip:N/A"
...
def main():
    ...
    # 检查网络接口是否存在；auto会在显示时按默认路由动态选择
    if str(config['network_interface']).lower() in ('auto', 'default'):
        logger.info("网络接口设置为自动选择")
    elif config['network_interface'] not in psutil.net_if_addrs():
